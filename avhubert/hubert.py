...
class AVHubertModel(BaseFairseqModel):
    ...
    def forward(
        ...
        # print("src_video", src_video.size())
        features_video = self.forward_features(src_video, modality='video')
        # print("features_video", features_video.size())
    ...
    def extract_finetune(self, source, padding_mask=None, mask=False, ret_conv=False, output_layer=None):
        ...
        # print("src_audio",src_audio.size())
        print("src_video",src_video.dtype)
        ...
        # print("features_concated",features.size())
        features = features.transpose(1, 2)
        features = self.layer_norm(features)
        ...
        if self.post_extract_proj is not None:
            features = self.post_extract_proj(features)
        # print("features_proj",features.size())
        features = self.dropout_input(features)
