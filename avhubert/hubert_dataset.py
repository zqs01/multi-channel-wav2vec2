...
class AVHubertDataset(FairseqDataset):
    ...
    def load_feature(self, mix_name):
        ...
        if 'audio' in self.modalities:
            ...
            wav_data = torch.tensor(wav_data, dtype=torch.float32).unsqueeze(0)
            # print(wav_data.size())
            audio_feats = kaldi.fbank(
                    wav_data,
                    num_mel_bins=26,
                ).numpy().astype(np.float32)
            # print("audio_feats", np.shape(audio_feats))

            
            # s1_time = time.time()
            audio_feats = stacker(audio_feats, self.stack_order_audio) # [T/stack_order_audio, F*stack_order_audio]
            # s2_time = time.time()
            # print("time1", s1_time - s_time)
            # print("time2", s2_time - s1_time)
    ...
    def collater(self, samples):
        ...
        if video_source is not None:
            # print("video_source", video_source[0].size())
            ...
            # print("collated_videos", collated_videos[0].size())
        else:
