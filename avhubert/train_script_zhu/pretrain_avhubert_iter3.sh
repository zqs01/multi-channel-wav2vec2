...
CUDA_VISIBLE_DEVICES=$cuda_devices python3 /apdcephfs/private_qiushizhu/av_hubert/fairseq/fairseq_cli/hydra_train.py \
       --config-dir conf/pretrain --config-name base_vox_iter3.yaml \
       task.data=/apdcephfs/share_1316500/qiushizhu/misp_related/misp_tsv_new/visual_audio/pretrain_hubert_iter2_tsv \
       task.label_dir=/apdcephfs/share_1316500/qiushizhu/misp_related/misp_tsv_new/visual_audio/pretrain_hubert_iter2_tsv \
       hydra.run.dir=/apdcephfs/private_qiushizhu/av_results/pretrain_avhubert_iter3_${all_world_size}ngpu_${updatefreq}updatefreq \
       common.user_dir=`pwd` \
