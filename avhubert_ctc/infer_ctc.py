...
def _main(cfg, output_file):
    ...
    generator = ViterbiDecoder(dictionary)

    num_sentences = 0
    has_target = True
    wps_meter = TimeMeter()
    result_dict = {'utt_id': [], 'ref': [], 'hypo': []}
    fhyp = open("{}/hyp.txt".format(cfg.common_eval.results_path), "w")
    fref = open("{}/ref.txt".format(cfg.common_eval.results_path), "w")
    ...
    for sample in progress:
        ...
        for batch_id, sample_id in enumerate(sample["id"].tolist()):
            ...
            hyp_pieces = dictionary.string(hypos[batch_id][0]["tokens"].int().cpu())
            # print(hyp_pieces)
            if "words" in hypos[batch_id][0]:
                hyp_words = " ".join(hypos[batch_id][0]["words"]) 
            else:
                hyp_words = hyp_pieces.replace("|", " ").replace(" ", "").strip()
            pattern = re.compile(r'([\u4e00-\u9fa5])')
            chars = pattern.split(hyp_words)
            chars = [w for w in chars if len(w.strip()) > 0]
            hyp_words = " ".join(chars)
            # print(hyp_words)
            result_dict['hypo'].append(hyp_words)
            fhyp.write(hyp_words + "\n")
            # Processes target.
            target_tokens = utils.strip_pad(toks, dictionary.pad())
            tgt_pieces = dictionary.string(target_tokens.int().cpu())
            tgt_words = post_process(tgt_pieces, "letter")
            result_dict['ref'].append(tgt_words)
            fref.write(tgt_words + "\n")
