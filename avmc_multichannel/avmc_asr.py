...
from .avmc import MASKING_DISTRIBUTION_CHOICES
...
@dataclass
class AVMCAsrConfig(FairseqDataclass):
    w2v_path: str = field(
...
@dataclass
class AVMCCtcConfig(AVMCAsrConfig):
    blank_weight: float = 0
    blank_mode: str = "add"


@register_model("avmc_ctc", dataclass=AVMCCtcConfig)
class AVMCCtc(BaseFairseqModel):
    def __init__(self, cfg: AVMCCtcConfig, w2v_encoder: BaseFairseqModel):
        super().__init__()
        self.cfg = cfg
    ...
    @classmethod
    def build_model(cls, cfg: AVMCCtcConfig, task: FairseqTask):
        """Build a new model instance."""
        w2v_encoder = AVMCEncoder(cfg, len(task.target_dictionary))
...
@dataclass
class AVMCSeq2SeqConfig(AVMCAsrConfig):
...
@register_model("avmc_seq2seq", dataclass=AVMCSeq2SeqConfig)
class AVMCSeq2SeqModel(FairseqEncoderDecoderModel):
    def __init__(self, encoder, decoder):
        super().__init__(encoder, decoder)

    @classmethod
    def build_model(cls, cfg: AVMCSeq2SeqConfig, task: FairseqTask):
        """Build a new model instance."""
        ...
        return AVMCSeq2SeqModel(encoder, decoder)

    @classmethod
    def build_encoder(cls, cfg: AVMCAsrConfig):
        return AVMCEncoder(cfg)

    @classmethod
    def build_decoder(cls, cfg: AVMCSeq2SeqConfig, tgt_dict, embed_tokens):
...
class AVMCEncoder(FairseqEncoder):
    def __init__(self, cfg: AVMCAsrConfig, output_size=None):
    ...
    def forward(self, audio_source0, audio_source1, audio_source2, audio_source3, audio_source4, audio_source5, video_source, audio_padding_mask, video_padding_mask, **kwargs):
    # def forward(self, source, padding_mask, **kwargs):

        w2v_args = {
            "audio_source0": audio_source0,
            "audio_source1": audio_source1,
            "audio_source2": audio_source2,
            "audio_source3": audio_source3,
            "audio_source4": audio_source4,
            "audio_source5": audio_source5,
